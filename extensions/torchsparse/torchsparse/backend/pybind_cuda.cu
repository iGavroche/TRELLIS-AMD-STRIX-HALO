...
// Conditional includes - implicit GEMM files have PTX assembly that doesn't
// work on HIP
...
#else
// Stub declarations for ROCm - these throw errors when called
at::Tensor conv_forward_implicit_gemm_cuda(
  ...
  AT_ERROR("conv_forward_implicit_gemm_cuda is not available on ROCm/HIP. Use "
           "gather-scatter backend instead.");
}
...
at::Tensor conv_forward_implicit_gemm_sorted_cuda(
  ...
  AT_ERROR("conv_forward_implicit_gemm_sorted_cuda is not available on "
           "ROCm/HIP. Use gather-scatter backend instead.");
}

at::Tensor conv_backward_wgrad_implicit_gemm_cuda(
    torch::Tensor _in_feats, torch::Tensor _kernel, torch::Tensor _out_in_map,
    int split_k_iters, bool allow_tf32, bool allow_fp16) {
  AT_ERROR("conv_backward_wgrad_implicit_gemm_cuda is not available on "
           "ROCm/HIP. Use gather-scatter backend instead.");
}

at::Tensor conv_backward_wgrad_implicit_gemm_sorted_cuda(
    torch::Tensor _in_feats, torch::Tensor _kernel, torch::Tensor _out_in_map,
    torch::Tensor _reduced_mask, torch::Tensor _reorder_loc, int split_k_iters,
    bool allow_tf32, bool allow_fp16) {
  AT_ERROR("conv_backward_wgrad_implicit_gemm_sorted_cuda is not available on "
           "ROCm/HIP. Use gather-scatter backend instead.");
}
...
at::Tensor conv_forward_fetch_on_demand_cuda(
    ...
    const int output_size, const bool transpose) {
  AT_ERROR("conv_forward_fetch_on_demand_cuda is not available on ROCm/HIP. "
           "Use gather-scatter backend instead.");
}
...
at::Tensor conv_forward_fetch_on_demand_no_fusion_cuda(
    ...
    at::Tensor neighbor_offset, at::Tensor input_mask, at::Tensor output_mask,
    const int output_size, const bool transpose) {
  AT_ERROR("conv_forward_fetch_on_demand_no_fusion_cuda is not available on "
           "ROCm/HIP. Use gather-scatter backend instead.");
}
#endif
