...
class GPUHashTable {
 ...
 public:
  GPUHashTable(const int capacity)
    ...
    srand(time(NULL));
    hipMalloc((void **)&table_keys, _capacity * sizeof(key_type));
    hipMemset(table_keys, 0, sizeof(key_type) * _capacity);
    hipMalloc((void **)&table_vals, _capacity * sizeof(val_type));
    hipMemset(table_vals, 0, sizeof(val_type) * _capacity);
  };
...
void GPUHashTable<key_type, val_type>::insert_many(const key_type *keys, const int n){
 hipLaunchKernelGGL(( insert_kernel<key_type, val_type>), dim3((n + BLOCK_SIZE - 1) / BLOCK_SIZE), dim3(BLOCK_SIZE), 0, 0, table_keys, table_vals, keys, n, _capacity);
}

template <typename key_type, typename val_type>
void GPUHashTable<key_type, val_type>::insert_many_coords(int *coords, const int n){
 hipLaunchKernelGGL(( insert_coords_kernel<key_type, val_type>), dim3((n + BLOCK_SIZE - 1) / BLOCK_SIZE), dim3(BLOCK_SIZE), 0, 0, table_keys, table_vals, coords, n, _capacity);
}
...
void GPUHashTable<key_type, val_type>::lookup_many(const key_type *keys, val_type *results, const int n){
 hipLaunchKernelGGL(( lookup_kernel<key_type, val_type>), dim3((n + BLOCK_SIZE - 1) / BLOCK_SIZE), dim3(BLOCK_SIZE), 0, 0, table_keys, table_vals, keys, results, n, _capacity);
}
...
void GPUHashTable<key_type, val_type>::lookup_many_coords(
  ...
  const int n, const int kernel_volume){
  if (kernel_volume % 2)
   hipLaunchKernelGGL(( lookup_coords_kernel<key_type, val_type, true>), dim3((n * kernel_volume + BLOCK_SIZE - 1) / BLOCK_SIZE), dim3(BLOCK_SIZE), 0, 0, 
      table_keys, table_vals, coords, results, kernel_sizes, strides,
      n, _capacity, kernel_volume);
  else
   hipLaunchKernelGGL(( lookup_coords_kernel<key_type, val_type, false>), dim3((n * kernel_volume + BLOCK_SIZE - 1) / BLOCK_SIZE), dim3(BLOCK_SIZE), 0, 0, 
      table_keys, table_vals, coords, results, kernel_sizes, strides,
      n, _capacity, kernel_volume);
}
